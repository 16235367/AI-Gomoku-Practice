...
class AI:
    ...
    def _evaluate_point_normal(self, board, x, y, z, player):
        ...
        for dx, dy, dz in dirs:
            ...
            blocks = (1 if f_blocked else 0) + (1 if b_blocked else 0)
    ...
    def get_best_move(self, board, player):
        ...
        for x, y, z in moves:
            if self.mode in ['2D', '3D_NORMAL']:
                ...
                center_bias = -((x - self.size // 2) ** 2 + (y - self.size // 2) ** 2 + (
                            z - (0 if self.mode == '2D' else self.size // 2)) ** 2) * 0.1
                total_score += center_bias + random.uniform(0, 2)
                ...
                # 【关键修复】：循环初始记录绝对纯净的副本状态
                orig_grid = copy.deepcopy(board.grid)
                orig_scores = board.scores.copy()
                ...
                # 2. 彻底还原后，再检查玩家落子能否得分
                board.grid = copy.deepcopy(orig_grid)
                board.scores = orig_scores.copy()

                board.grid[z][y][x] = hu_player
                board._process_chains()
                hu_scored = board.scores[hu_player] > orig_scores[hu_player]

                # 3. 再次彻底还原为纯净状态，调用打分系统
                board.grid = orig_grid
                board.scores = orig_scores

                if hu_scored:
                    total_score = 50000  # 玩家能得分，强制必须拦截
                else:
                    total_score = self._evaluate_point_melt(board, x, y, z, player) + self._evaluate_point_melt(board,
                                                                                                                x, y, z,
                                                                                                                hu_player) * 1.1
                    center_bias = -((x - 7) ** 2 + (y - 7) ** 2 + (z - 7) ** 2) * 0.1
                    total_score += center_bias + random.uniform(0, 2)
...
def play_piece(req: PlayRequest):
    ...
    return {
        "success": success,
        "isWin": b.check_win(req.player) if success else False,
        "state": b.grid,
        "scores": b.scores,
...
def undo_move():
    ...
    return {
        "success": success,
        "state": b.grid if success else [],
        "scores": b.scores if success else {},
        "lastMove": b.history[-1][2] if b.history else None
    }